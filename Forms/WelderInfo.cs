...
namespace DataBaseA
{
    public partial class WelderInfo : Form
    {
        ...
        public WelderInfo()
        {
            ...
            CertificateModel test = new CertificateModel();
        }
        ...
        private void WelderInfo_Load(object sender, EventArgs e)
        {
            ...
            // --- TEST: Does CertificateModel compile? ---
            var testCert = new DataBaseA.Models.CertificateModel();
            testCert.CertificateName = "Test Certificate";
            // You can leave it unused, this is just to check compilation
        }
        ...
        {
            if (dataGridViewCertificates.CurrentRow == null)
            {
                MessageBox.Show("Please select a certificate first.");
                return;
            }

            int certificateId = Convert.ToInt32(
                dataGridViewCertificates.CurrentRow.Cells["CertificateID"].Value);

            string connString = ConfigurationManager
                .ConnectionStrings["DataBaseA.Properties.Settings.DatabaseAConnectionString"]
                .ConnectionString;

            var repository = new CertificateRepository(connString);
            var certificateService = new CertificateService(repository);
            var pdfService = new PdfService();

            var certificate = certificateService.GetFullCertificate(certificateId);

            using (SaveFileDialog sfd = new SaveFileDialog())
            {
                sfd.Filter = "PDF files (*.pdf)|*.pdf";
                sfd.FileName = certificate.CertificateName + ".pdf";

                if (sfd.ShowDialog() == DialogResult.OK)
                {
                    pdfService.GenerateCertificatePdf(certificate, sfd.FileName);
